...
### Deploy to Render (Free Tier) for FE Dev Testing
Added a multi-stage `Dockerfile` (`eclipse-temurin:21-jdk` build stage → `eclipse-temurin:21-jre` runtime, since Render has no native Java buildpack) and a `.dockerignore`. Fixed `server.port` in `application.yaml` to read `${PORT:${SERVER_PORT:8080}}` so Render's injected `PORT` env var wins while local dev keeps using `SERVER_PORT`/`8080`. No API/endpoint/schema changes — this reuses the existing Neon dev DB rather than Render's own free Postgres (which auto-expires after 90 days). Verified locally end-to-end: `docker build` succeeds, and a full run against a real throwaway Postgres container confirmed Flyway migrates cleanly, Tomcat binds to the `PORT`-supplied port, and `GET /api/v1/vocabularies` returns 200. Actually creating the Render Web Service and setting its env vars (`DB_URL`/`DB_USERNAME`/`DB_PASSWORD`/`JWT_SECRET`/`JWT_EXPIRATION_SECONDS`/`JWT_REFRESH_EXPIRATION_SECONDS`/`CORS_ALLOWED_ORIGINS`/`GOOGLE_CLIENT_ID`) is a manual dashboard step outside this repo, still pending. Open consideration for later: Render free tier sleeps after ~15 min idle (30-60s+ JVM cold start on wake), and Neon's free tier also scales-to-zero, so the two could stack into a slow first request — worth confirming this is tolerable before relying on it daily.
